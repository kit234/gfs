...
if [ "${GFS_Option}" == "source" ]; then
    ...
    axel https://github.com/wxWidgets/wxWidgets/releases/download/v3.1.2/wxWidgets-3.1.2.tar.bz2 -o ${GFS_Software_Source_Path}/software_tar
    tar -xvf ${GFS_Software_Source_Path}/software_tar/wxWidgets-3.1.2.tar.bz2 -C ${GFS_Software_Source_Path}
    echo "已将wxwidgets的压缩包下载至${GFS_Software_Source_Path}/software_tar"
    echo "已将wxwidgets的压缩包解压至${GFS_Software_Source_Path}"
fi
if [ "${GFS_Option}" == "configure" ]; then
    if [ ! -f "${GFS_Software_Source_Path}/wxWidgets-3.1.2/configure" ]; then
        echo "文件${GFS_Software_Source_Path}/wxWidgets-3.1.2/configure不存在"
        unset GFS_Software_Source_Path GFS_Option
        exit
    fi
    cd ${GFS_Software_Source_Path}/wxWidgets-3.1.2
    while true ; do 
...
if [ "${GFS_Option}" == "make" ]; then
    if [ ! -f "${GFS_Software_Source_Path}/wxWidgets-3.1.2/Makefile" ]; then
        echo "文件${GFS_Software_Source_Path}/wxWidgets-3.1.2/Makefile不存在"
...
if [ "${GFS_Option}" == "remove" ]; then
    rm -r ${GFS_Software_Source_Path}/wxWidgets-3.1.2
    rm ${GFS_Software_Source_Path}/software_tar/wxWidgets-3.1.2.tar.bz2
fi
unset GFS_Option
