...
if [ "${GFS_Option}" == "source" ]; then
    ...
    axel http://mirrors.ustc.edu.cn/gnu/emacs/emacs-26.1.tar.gz -o ${GFS_Software_Source_Path}/software_tar
    tar -xvf ${GFS_Software_Source_Path}/software_tar/emacs-26.1.tar.gz -C ${GFS_Software_Source_Path}
    echo "已将emacs的压缩包下载至${GFS_Software_Source_Path}/software_tar"
    echo "已将emacs的压缩包解压至${GFS_Software_Source_Path}"
fi
if [ "${GFS_Option}" == "configure" ]; then
    if [ ! -f "${GFS_Software_Source_Path}/emacs-26.1/configure" ]; then
        echo "文件${GFS_Software_Source_Path}/emacs-26.1/configure不存在"
        unset GFS_Software_Source_Path GFS_Option
        exit
    fi
    cd ${GFS_Software_Source_Path}/emacs-26.1
    while true ; do 
...
if [ "${GFS_Option}" == "make" ]; then
    if [ ! -f "${GFS_Software_Source_Path}/emacs-26.1/Makefile" ]; then
        echo "文件${GFS_Software_Source_Path}/emacs-26.1/Makefile不存在"
...
if [ "${GFS_Option}" == "remove" ]; then
    rm -r ${GFS_Software_Source_Path}/emacs-26.1
    rm ${GFS_Software_Source_Path}/software_tar/emacs-26.1.tar.gz
fi
unset GFS_Option
